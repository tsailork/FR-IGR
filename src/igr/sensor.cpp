...
static constexpr double DIVERGENCE_THRESHOLD =
    9.0e19; //-0.0; // Must be compressive
static constexpr double SENSOR_THRESHOLD =
    0.0; // 0.1; // Threshold for raw sensor 'val'
          ...
          if (USE_PRESSURE_SENSOR) {
            ...
            for (int iy = 0; iy < p.N_PTS; ++iy) {
              for (int ix = 0; ix < p.N_PTS; ++ix) {
                ...
                dp_dx[iy * p.N_PTS + ix] = dpdx * (2.0 / b.dx);
              }
            }
            for (int ix = 0; ix < p.N_PTS; ++ix) {
              for (int iy = 0; iy < p.N_PTS; ++iy) {
                ...
                dp_dy[iy * p.N_PTS + ix] = dpdy * (2.0 / b.dy);
              }
            }
            ...
            for (int iy = 0; iy < p.N_PTS; ++iy)
              for (int ix = 0; ix < p.N_PTS; ++ix) {
                ...
                double val =
                    (dp_dx[idx] * dp_dx[idx] + dp_dy[idx] * dp_dy[idx]) /
                    (p_ref * p_ref);
                b.S_buf[b.get_flat_idx(ey, ex, iy, ix, p.N_PTS)] =
            ...
            for (int iy = 0; iy < p.N_PTS; ++iy) {
              for (int ix = 0; ix < p.N_PTS; ++ix) {
                ...
                if (compression < DIVERGENCE_THRESHOLD &&
                  ...
                  b.S_buf[b.get_flat_idx(ey, ex, iy, ix, p.N_PTS)] = 0.0;
                }
              }
            }
          }

...
void Solver::compute_entropic_pressure() {
  ...
  // -----------------------------------------------------------------
  // σ-clamp: bound entropic pressure by local physical pressure.
  //
  // The entropic pressure σ appears in both the momentum flux and the
  // energy flux. While σ acts to dissipate kinetic energy into
  // entropic pressure, including it in the energy flux ensures that
  // the total energy (including the work done by σ) is conserved.
  //
  // Clamping σ ≤ p at every point ensures the total effective

/**
 * @file implicit_ilu.cpp
 * @brief Block-ILU(0) Preconditioner and Factorization Engine for 2D High-Order FR-DG.
 */

#include "implicit_ilu.hpp"
#include "implicit_precond.hpp"
#include <cmath>
#include <algorithm>
#include <iostream>
#ifdef _OPENMP
#include <omp.h>
#endif

namespace Implicit {

// Dense matrix-matrix multiply: C = C - A * B (Size n x n)
static void block_gemm_sub(int n, const double* A, const double* B, double* C) {
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            double sum = 0.0;
            for (int k = 0; k < n; ++k) {
                sum += A[i * n + k] * B[k * n + j];
            }
            C[i * n + j] -= sum;
        }
    }
}

// Dense matrix-vector multiply-subtract: w = w - A * v (Size n)
static void block_gemv_sub(int n, const double* A, const double* v, double* w) {
    for (int i = 0; i < n; ++i) {
        double sum = 0.0;
        for (int j = 0; j < n; ++j) {
            sum += A[i * n + j] * v[j];
        }
        w[i] -= sum;
    }
}

static bool invert_block_matrix_ilu(int n, const double* A, double* A_inv) {
    alignas(64) double mat[8192];
    std::fill(mat, mat + n * n * 2, 0.0);
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            mat[i * (2 * n) + j] = A[i * n + j];
        }
        mat[i * (2 * n) + n + i] = 1.0;
    }
    for (int i = 0; i < n; ++i) {
        int pivot = i;
        double max_val = std::abs(mat[i * (2 * n) + i]);
        for (int k = i + 1; k < n; ++k) {
            double val = std::abs(mat[k * (2 * n) + i]);
            if (val > max_val) {
                max_val = val;
                pivot = k;
            }
        }
        if (max_val < 1e-14) return false;
        if (pivot != i) {
            for (int j = 0; j < 2 * n; ++j) {
                std::swap(mat[i * (2 * n) + j], mat[pivot * (2 * n) + j]);
            }
        }
        double diag = mat[i * (2 * n) + i];
        for (int j = 0; j < 2 * n; ++j) {
            mat[i * (2 * n) + j] /= diag;
        }
        for (int k = 0; k < n; ++k) {
            if (k != i) {
                double factor = mat[k * (2 * n) + i];
                for (int j = 0; j < 2 * n; ++j) {
                    mat[k * (2 * n) + j] -= factor * mat[i * (2 * n) + j];
                }
            }
        }
    }
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            A_inv[i * n + j] = mat[i * (2 * n) + n + j];
        }
    }
    return true;
}

void BlockILUPreconditioner2D::build(
    const std::vector<CellDim<2>*>& cells,
    const Basis& basis,
    double gamma_fluid,
    double dt,
    double gamma_stage
) {
    n_cells = cells.size();
    if (n_cells == 0) return;

    int npts = static_cast<int>(basis.dgl.size());
    block_size = npts * npts * 4;

    inv_U_diag.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    L_west.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    L_south.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    L_east.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    L_north.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    A_west.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    A_east.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    A_south.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));
    A_north.assign(n_cells, std::vector<double>(block_size * block_size, 0.0));

    west_neigh.assign(n_cells, -1);
    east_neigh.assign(n_cells, -1);
    south_neigh.assign(n_cells, -1);
    north_neigh.assign(n_cells, -1);
    cell_level.assign(n_cells, 0);

    for (int idx = 0; idx < n_cells; ++idx) {
        const auto* c = cells[idx];
        cell_level[idx] = c->ex + c->ey;
        if (c->neighbors[0]) west_neigh[idx]  = c->neighbors[0]->cell_index;
        if (c->neighbors[1]) east_neigh[idx]  = c->neighbors[1]->cell_index;
        if (c->neighbors[2]) south_neigh[idx] = c->neighbors[2]->cell_index;
        if (c->neighbors[3]) north_neigh[idx] = c->neighbors[3]->cell_index;
    }

    // Build Wavefront levels: Breadth-First traversal from cell 0 or level (ex + ey)
    int max_level = 0;
    for (int idx = 0; idx < n_cells; ++idx) {
        max_level = std::max(max_level, cells[idx]->ex + cells[idx]->ey);
    }
    levels.assign(max_level + 1, std::vector<int>());
    for (int idx = 0; idx < n_cells; ++idx) {
        int lvl = std::clamp(cells[idx]->ex + cells[idx]->ey, 0, max_level);
        levels[lvl].push_back(idx);
    }

    // Step 1: Assemble Local Diagonal Blocks A_ee and Off-Diagonal Neighbor Blocks
    std::vector<std::vector<double>> A_diag(n_cells, std::vector<double>(block_size * block_size, 0.0));

    #pragma omp parallel for schedule(static)
    for (int c_idx = 0; c_idx < n_cells; ++c_idx) {
        const CellDim<2>* c = cells[c_idx];
        double dx = c->dx;
        double dy = c->dy;

        std::vector<double> dR_dU(block_size * block_size, 0.0);

        // Precompute Rusanov maximum wave speeds at interface faces
        double lam_R = 0.0, lam_L = 0.0, lam_T = 0.0, lam_B = 0.0;
        for (int iy = 0; iy < npts; ++iy) {
            for (int ix = 0; ix < npts; ++ix) {
                int idx = iy * npts + ix;
                double rho = std::max(c->U[0 * npts * npts + idx], 1e-12);
                double u = c->U[1 * npts * npts + idx] / rho;
                double v = c->U[2 * npts * npts + idx] / rho;
                double E = c->U[3 * npts * npts + idx];
                double P = std::max((gamma_fluid - 1.0) * (E - 0.5 * rho * (u * u + v * v)), 1e-12);
                double a = std::sqrt(gamma_fluid * P / rho);

                lam_R = std::max(lam_R, std::abs(u) + a);
                lam_L = std::max(lam_L, std::abs(u) + a);
                lam_T = std::max(lam_T, std::abs(v) + a);
                lam_B = std::max(lam_B, std::abs(v) + a);
            }
        }

        // 1. Diagonal Block A_{e,e}
        for (int iy_j = 0; iy_j < npts; ++iy_j) {
            for (int ix_j = 0; ix_j < npts; ++ix_j) {
                int node_j = iy_j * npts + ix_j;
                double U_node_j[4];
                for (int v = 0; v < 4; ++v) U_node_j[v] = c->U[v * npts * npts + node_j];
                double A_j[16], B_j[16];
                compute_euler_jacobian_x_2d(U_node_j, gamma_fluid, A_j);
                compute_euler_jacobian_y_2d(U_node_j, gamma_fluid, B_j);

                for (int iy_i = 0; iy_i < npts; ++iy_i) {
                    for (int ix_i = 0; ix_i < npts; ++ix_i) {
                        int node_i = iy_i * npts + ix_i;

                        // X-sweep FR-DG Jacobian block
                        if (iy_i == iy_j) {
                            double inv_hx2 = 2.0 / dx;
                            double d_ij = basis.D(ix_i, ix_j);
                            double dgl_i = basis.dgl[ix_i];
                            double dgr_i = basis.dgr[ix_i];
                            double lL_j  = basis.l_L[ix_j];
                            double lR_j  = basis.l_R[ix_j];

                            for (int r = 0; r < 4; ++r) {
                                for (int col = 0; col < 4; ++col) {
                                    int row_idx = (node_i * 4 + r);
                                    int col_idx = (node_j * 4 + col);
                                    double val_A = A_j[r * 4 + col];
                                    double eye = (r == col) ? 1.0 : 0.0;
                                    double dR_x = -inv_hx2 * (
                                        d_ij * val_A +
                                        dgl_i * 0.5 * (-val_A - lam_L * eye) * lL_j +
                                        dgr_i * 0.5 * (-val_A + lam_R * eye) * lR_j
                                    );
                                    dR_dU[row_idx * block_size + col_idx] += dR_x;
                                }
                            }
                        }

                        // Y-sweep FR-DG Jacobian block
                        if (ix_i == ix_j) {
                            double inv_hy2 = 2.0 / dy;
                            double d_ij = basis.D(iy_i, iy_j);
                            double dgl_i = basis.dgl[iy_i];
                            double dgr_i = basis.dgr[iy_i];
                            double lL_j  = basis.l_L[iy_j];
                            double lR_j  = basis.l_R[iy_j];

                            for (int r = 0; r < 4; ++r) {
                                for (int col = 0; col < 4; ++col) {
                                    int row_idx = (node_i * 4 + r);
                                    int col_idx = (node_j * 4 + col);
                                    double val_B = B_j[r * 4 + col];
                                    double eye = (r == col) ? 1.0 : 0.0;
                                    double dR_y = -inv_hy2 * (
                                        d_ij * val_B +
                                        dgl_i * 0.5 * (-val_B - lam_B * eye) * lL_j +
                                        dgr_i * 0.5 * (-val_B + lam_T * eye) * lR_j
                                    );
                                    dR_dU[row_idx * block_size + col_idx] += dR_y;
                                }
                            }
                        }
                    }
                }
            }
        }

        // Form A_diag = I - gamma_stage * dt * dR_dU
        for (int i = 0; i < block_size; ++i) {
            for (int j = 0; j < block_size; ++j) {
                double eye = (i == j) ? 1.0 : 0.0;
                A_diag[c_idx][i * block_size + j] = eye - (gamma_stage * dt) * dR_dU[i * block_size + j];
            }
        }

        // 2. Off-Diagonal West Neighbor Jacobian A_{e, west}
        int w_idx = west_neigh[c_idx];
        if (w_idx >= 0) {
            const CellDim<2>* c_w = cells[w_idx];
            double inv_hx2 = 2.0 / dx;
            for (int iy_j = 0; iy_j < npts; ++iy_j) {
                for (int ix_j = 0; ix_j < npts; ++ix_j) {
                    int node_j = iy_j * npts + ix_j;
                    double U_w[4];
                    for (int v = 0; v < 4; ++v) U_w[v] = c_w->U[v * npts * npts + node_j];
                    double A_w[16];
                    compute_euler_jacobian_x_2d(U_w, gamma_fluid, A_w);

                    for (int ix_i = 0; ix_i < npts; ++ix_i) {
                        int node_i = iy_j * npts + ix_i;
                        double dgl_i = basis.dgl[ix_i];
                        double lR_j  = basis.l_R[ix_j];

                        for (int r = 0; r < 4; ++r) {
                            for (int col = 0; col < 4; ++col) {
                                int row_idx = (node_i * 4 + r);
                                int col_idx = (node_j * 4 + col);
                                double val_A = A_w[r * 4 + col];
                                double eye = (r == col) ? 1.0 : 0.0;
                                double dR_w = -inv_hx2 * (dgl_i * 0.5 * (val_A + lam_L * eye) * lR_j);
                                A_west[c_idx][row_idx * block_size + col_idx] = -(gamma_stage * dt) * dR_w;
                            }
                        }
                    }
                }
            }
        }

        // 3. Off-Diagonal South Neighbor Jacobian A_{e, south}
        int s_idx = south_neigh[c_idx];
        if (s_idx >= 0) {
            const CellDim<2>* c_s = cells[s_idx];
            double inv_hy2 = 2.0 / dy;
            for (int iy_j = 0; iy_j < npts; ++iy_j) {
                for (int ix_j = 0; ix_j < npts; ++ix_j) {
                    int node_j = iy_j * npts + ix_j;
                    double U_s[4];
                    for (int v = 0; v < 4; ++v) U_s[v] = c_s->U[v * npts * npts + node_j];
                    double B_s[16];
                    compute_euler_jacobian_y_2d(U_s, gamma_fluid, B_s);

                    for (int iy_i = 0; iy_i < npts; ++iy_i) {
                        int node_i = iy_i * npts + ix_j;
                        double dgl_i = basis.dgl[iy_i];
                        double lR_j  = basis.l_R[iy_j];

                        for (int r = 0; r < 4; ++r) {
                            for (int col = 0; col < 4; ++col) {
                                int row_idx = (node_i * 4 + r);
                                int col_idx = (node_j * 4 + col);
                                double val_B = B_s[r * 4 + col];
                                double eye = (r == col) ? 1.0 : 0.0;
                                double dR_s = -inv_hy2 * (dgl_i * 0.5 * (val_B + lam_B * eye) * lR_j);
                                A_south[c_idx][row_idx * block_size + col_idx] = -(gamma_stage * dt) * dR_s;
                            }
                        }
                    }
                }
            }
        }

        // 4. Off-Diagonal East Neighbor Jacobian A_{e, east}
        int e_idx = east_neigh[c_idx];
        if (e_idx >= 0) {
            const CellDim<2>* c_e = cells[e_idx];
            double inv_hx2 = 2.0 / dx;
            for (int iy_j = 0; iy_j < npts; ++iy_j) {
                for (int ix_j = 0; ix_j < npts; ++ix_j) {
                    int node_j = iy_j * npts + ix_j;
                    double U_e[4];
                    for (int v = 0; v < 4; ++v) U_e[v] = c_e->U[v * npts * npts + node_j];
                    double A_e_val[16];
                    compute_euler_jacobian_x_2d(U_e, gamma_fluid, A_e_val);

                    for (int ix_i = 0; ix_i < npts; ++ix_i) {
                        int node_i = iy_j * npts + ix_i;
                        double dgr_i = basis.dgr[ix_i];
                        double lL_j  = basis.l_L[ix_j];

                        for (int r = 0; r < 4; ++r) {
                            for (int col = 0; col < 4; ++col) {
                                int row_idx = (node_i * 4 + r);
                                int col_idx = (node_j * 4 + col);
                                double val_A = A_e_val[r * 4 + col];
                                double eye = (r == col) ? 1.0 : 0.0;
                                double dR_e_jac = -inv_hx2 * (dgr_i * 0.5 * (val_A - lam_R * eye) * lL_j);
                                A_east[c_idx][row_idx * block_size + col_idx] = -(gamma_stage * dt) * dR_e_jac;
                            }
                        }
                    }
                }
            }
        }

        // 5. Off-Diagonal North Neighbor Jacobian A_{e, north}
        int n_idx = north_neigh[c_idx];
        if (n_idx >= 0) {
            const CellDim<2>* c_n = cells[n_idx];
            double inv_hy2 = 2.0 / dy;
            for (int iy_j = 0; iy_j < npts; ++iy_j) {
                for (int ix_j = 0; ix_j < npts; ++ix_j) {
                    int node_j = iy_j * npts + ix_j;
                    double U_n_val[4];
                    for (int v = 0; v < 4; ++v) U_n_val[v] = c_n->U[v * npts * npts + node_j];
                    double B_n[16];
                    compute_euler_jacobian_y_2d(U_n_val, gamma_fluid, B_n);

                    for (int iy_i = 0; iy_i < npts; ++iy_i) {
                        int node_i = iy_i * npts + ix_j;
                        double dgr_i = basis.dgr[iy_i];
                        double lL_j  = basis.l_L[iy_j];

                        for (int r = 0; r < 4; ++r) {
                            for (int col = 0; col < 4; ++col) {
                                int row_idx = (node_i * 4 + r);
                                int col_idx = (node_j * 4 + col);
                                double val_B = B_n[r * 4 + col];
                                double eye = (r == col) ? 1.0 : 0.0;
                                double dR_n = -inv_hy2 * (dgr_i * 0.5 * (val_B - lam_T * eye) * lL_j);
                                A_north[c_idx][row_idx * block_size + col_idx] = -(gamma_stage * dt) * dR_n;
                            }
                        }
                    }
                }
            }
        }
    }

    // Step 2: In-Place Block-ILU(0) Factorization Sweep
    // For level k = 0 ... max_level, process diagonal cells in wavefront order
    for (size_t lvl = 0; lvl < levels.size(); ++lvl) {
        #pragma omp parallel for schedule(static)
        for (size_t c_pos = 0; c_pos < levels[lvl].size(); ++c_pos) {
            int c_idx = levels[lvl][c_pos];
            std::vector<double> M_e = A_diag[c_idx];

            int w_idx = west_neigh[c_idx];
            if (w_idx >= 0 && cell_level[w_idx] < cell_level[c_idx]) {
                for (int i = 0; i < block_size; ++i) {
                    for (int j = 0; j < block_size; ++j) {
                        double sum = 0.0;
                        for (int k = 0; k < block_size; ++k) {
                            sum += A_west[c_idx][i * block_size + k] * inv_U_diag[w_idx][k * block_size + j];
                        }
                        L_west[c_idx][i * block_size + j] = sum;
                    }
                }
                // M_e = M_e - L_{e, west} * A_{west, east}
                block_gemm_sub(block_size, L_west[c_idx].data(), A_east[w_idx].data(), M_e.data());
            }

            int s_idx = south_neigh[c_idx];
            if (s_idx >= 0 && cell_level[s_idx] < cell_level[c_idx]) {
                // Compute L_south[c_idx] = A_{e, south} * U_{south, south}^{-1}
                for (int i = 0; i < block_size; ++i) {
                    for (int j = 0; j < block_size; ++j) {
                        double sum = 0.0;
                        for (int k = 0; k < block_size; ++k) {
                            sum += A_south[c_idx][i * block_size + k] * inv_U_diag[s_idx][k * block_size + j];
                        }
                        L_south[c_idx][i * block_size + j] = sum;
                    }
                }
                // M_e = M_e - L_{e, south} * A_{south, north}
                block_gemm_sub(block_size, L_south[c_idx].data(), A_north[s_idx].data(), M_e.data());
            }

            // Invert M_e -> inv_U_diag[c_idx]
            if (!invert_block_matrix_ilu(block_size, M_e.data(), inv_U_diag[c_idx].data())) {
                for (int i = 0; i < block_size; ++i) {
                    inv_U_diag[c_idx][i * block_size + i] = 1.0;
                }
            }
        }
    }
}

void BlockILUPreconditioner2D::apply(const std::vector<double>& v_in, std::vector<double>& w_out) const {
    w_out.assign(v_in.size(), 0.0);
    if (n_cells == 0) return;

    std::vector<double> y_vec(v_in.size(), 0.0);

    // Step 1: Forward Substitution Pass (L * Y = v_in)
    for (size_t lvl = 0; lvl < levels.size(); ++lvl) {
        int n_lvl = static_cast<int>(levels[lvl].size());
        #pragma omp parallel for schedule(static) if(n_lvl > 64)
        for (int c_pos = 0; c_pos < n_lvl; ++c_pos) {
            int c_idx = levels[lvl][c_pos];
            int offset = c_idx * block_size;

            alignas(64) double v_eff[256];
            for (int i = 0; i < block_size; ++i) v_eff[i] = v_in[offset + i];

            int w_idx = west_neigh[c_idx];
            if (w_idx >= 0 && cell_level[w_idx] < cell_level[c_idx]) {
                block_gemv_sub(block_size, L_west[c_idx].data(), &y_vec[w_idx * block_size], v_eff);
            }

            int s_idx = south_neigh[c_idx];
            if (s_idx >= 0 && cell_level[s_idx] < cell_level[c_idx]) {
                block_gemv_sub(block_size, L_south[c_idx].data(), &y_vec[s_idx * block_size], v_eff);
            }

            int e_idx = east_neigh[c_idx];
            if (e_idx >= 0 && cell_level[e_idx] < cell_level[c_idx]) {
                block_gemv_sub(block_size, L_east[c_idx].data(), &y_vec[e_idx * block_size], v_eff);
            }

            int n_idx = north_neigh[c_idx];
            if (n_idx >= 0 && cell_level[n_idx] < cell_level[c_idx]) {
                block_gemv_sub(block_size, L_north[c_idx].data(), &y_vec[n_idx * block_size], v_eff);
            }

            for (int i = 0; i < block_size; ++i) {
                y_vec[offset + i] = v_eff[i];
            }
        }
    }

    // Step 2: Backward Substitution Pass (U * w_out = Y)
    for (int lvl = static_cast<int>(levels.size()) - 1; lvl >= 0; --lvl) {
        int n_lvl = static_cast<int>(levels[lvl].size());
        #pragma omp parallel for schedule(static) if(n_lvl > 64)
        for (int c_pos = 0; c_pos < n_lvl; ++c_pos) {
            int c_idx = levels[lvl][c_pos];
            int offset = c_idx * block_size;

            alignas(64) double y_eff[256];
            for (int i = 0; i < block_size; ++i) y_eff[i] = y_vec[offset + i];

            int w_idx = west_neigh[c_idx];
            if (w_idx >= 0 && cell_level[w_idx] > cell_level[c_idx]) {
                block_gemv_sub(block_size, A_west[c_idx].data(), &w_out[w_idx * block_size], y_eff);
            }

            int s_idx = south_neigh[c_idx];
            if (s_idx >= 0 && cell_level[s_idx] > cell_level[c_idx]) {
                block_gemv_sub(block_size, A_south[c_idx].data(), &w_out[s_idx * block_size], y_eff);
            }

            int e_idx = east_neigh[c_idx];
            if (e_idx >= 0 && cell_level[e_idx] > cell_level[c_idx]) {
                block_gemv_sub(block_size, A_east[c_idx].data(), &w_out[e_idx * block_size], y_eff);
            }

            int n_idx = north_neigh[c_idx];
            if (n_idx >= 0 && cell_level[n_idx] > cell_level[c_idx]) {
                block_gemv_sub(block_size, A_north[c_idx].data(), &w_out[n_idx * block_size], y_eff);
            }

            for (int i = 0; i < block_size; ++i) {
                double sum = 0.0;
                for (int j = 0; j < block_size; ++j) {
                    sum += inv_U_diag[c_idx][i * block_size + j] * y_eff[j];
                }
                w_out[offset + i] = sum;
            }
        }
    }


}



} // namespace Implicit
